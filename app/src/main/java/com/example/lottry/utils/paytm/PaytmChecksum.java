package com.example.lottry.utils.paytm;
...
//        byte[] baseEncodedByte = new byte[0];
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5PADDING");
        cipher.init(1, new SecretKeySpec(key.getBytes(), "AES"), new IvParameterSpec(ivParamBytes));
        byte[] baseEncodedByte = Base64.encode(cipher.doFinal(input.getBytes()),Base64.DEFAULT);


//        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
//             baseEncodedByte = Base64.getEncoder().encode(cipher.doFinal(input.getBytes()));
//        }else {
//
//            baseEncodedByte=Base64.getEncoder().encodeToString(cipher.doFinal(input.getBytes())).getBytes();
//        }
    ...
    public static String decrypt(String input, String key) throws InvalidAlgorithmParameterException, InvalidKeyException, NoSuchPaddingException, NoSuchAlgorithmException, NoSuchProviderException, IOException, BadPaddingException, IllegalBlockSizeException {
        ...
        byte[] baseDecodedByte = Base64.decode(input,Base64.DEFAULT);
